...
def window_evaluation_and_append_metrics(
    ...
    NRMSE_window_metric, RMSE_window_metric, MAE_window_metric, MAPE_window_metric, CORR_window_metric = window_evaluation(
        config,
        actuals,
        inverse_transformed_predictions,
    ...
    if node_id > args.num_nodes - 2 and index == 3:
        ...
        for metric_dict__, filename in metric_dicts_and_filenames:
            for abbr__ in metric_dict__:
                NRMSE_mean = np.mean(metric_dict__[abbr__]["NRMSE_LIST"])
                ...
                if mean_wk in filename and not np.isnan(RMSE_mean):
                    ...
                    with open(csv_output_file,
                        ...
                        writer.writerow([
                            ...
                            MAPE_mean, CORR_mean, NRMSE_mean,
                        ])
                        print(
                            "State, Model, RMSE, MAE, MAPE, CORR, NRMSE",
                            abbr__,
                            name,
...
def append_window_metrics(week_metrics, abbr, NRMSE, RMSE, MAE, MAPE, CORR):
    week_metrics[abbr]["NRMSE_LIST"].append(NRMSE)
...
def apply_mean_calculation(config, args, csv_file_name):
    column_names = [
        "abbr", "name",  "RMSE_mean", "MAE_mean", "MAPE_mean",
