...
def Test(config, model_, test_loader, adjs, scaler_name,
            ...
            for node_id, abbr in enumerate(list_of_abbr):
                ...
                preds_dict[abbr][start_date_][
                    "ACTUAL_past"] = combined_tar[:past_steps, :].reshape(
                        1, -1)
                preds_dict[abbr][start_date_]["ACTUAL_future"] = combined_tar[
                        ...
                        for abbr__ in metric_dict__:
                            ...
                            with open(file_path, "w",
                                ...
                                json.dump(serialize_dict(metric_dict__),
                                          json_file,
                                          indent=4)
                            if "6wk" in filename and not np.isnan(RMSE_mean):
                                ...
                                print("State, Model, RMSE, MAE, MAPE, CORR",
                                      abbr__, name, RMSE_mean, MAE_mean,
                                      MAPE_mean, CORR_mean)
                                file.flush()
    ...
    averages_df.to_csv(csv_file_name.replace(".csv", "_mean.csv"),
                       index=False,
                       encoding="utf-8")
